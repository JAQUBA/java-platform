...
def generate_pom_xml():
    ...
    try:
        project_name = project_config.get(env_section, "project_name") if project_config.has_option(env_section, "project_name") else "java-project"
        print(f"🔍 DEBUG: Project name: {project_name}")
    except:
        project_name = "java-project"
    
    try:
        project_version = project_config.get(env_section, "project_version") if project_config.has_option(env_section, "project_version") else "1.0.0"
        print(f"🔍 DEBUG: Project version: {project_version}")
    except:
        project_version = "1.0.0"
    
    try:
        project_description = project_config.get(env_section, "project_description") if project_config.has_option(env_section, "project_description") else "Java project built with PlatformIO"
        print(f"🔍 DEBUG: Project description: {project_description}")
    except:
        project_description = "Java project built with PlatformIO"
    
    try:
        extra_repositories = project_config.get(env_section, "extra_repositories") if project_config.has_option(env_section, "extra_repositories") else ""
        print(f"🔍 DEBUG: Extra repositories: {repr(extra_repositories)}")
    except:
        extra_repositories = ""
    ...
    compiler_target = "18"
    try:
        if project_config.has_option(env_section, "build_flags"):
            build_flags = project_config.get(env_section, "build_flags")
            print(f"🔍 DEBUG: Found build_flags: {repr(build_flags)}")
            for flag in build_flags.split('\n'):
                flag = flag.strip()
                if flag.startswith('-Dmaven.compiler.source='):
                    compiler_source = flag.split('=')[1]
                    print(f"🔍 DEBUG: Set compiler source to: {compiler_source}")
                elif flag.startswith('-Dmaven.compiler.target='):
                    compiler_target = flag.split('=')[1]
                    print(f"🔍 DEBUG: Set compiler target to: {compiler_target}")
        else:
            print(f"⚠️ DEBUG: No build_flags found in {env_section}")
    except Exception as e:
        print(f"❌ DEBUG: Error reading build_flags: {e}")
...
'''
    ...
    pom_file_path = join(env.get("PROJECT_DIR"), "pom.xml")
    with open(pom_file_path, 'w') as f:
        f.write(pom_content)
    
    print(f"✅ Generated pom.xml: {pom_file_path}")
